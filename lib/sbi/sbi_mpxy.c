...
assert_field_offset(msg_max_len, SBI_MPXY_ATTR_MSG_MAX_LEN);
...
{
	int ret;
	void *msgbuf, *respbuf, *shmem_base;
	...
	if (msg_data_len > rs->shmem.shmem_size ||
		msg_data_len > channel->attrs.msg_max_len)
		return SBI_ERR_INVALID_PARAM;

	shmem_base = hart_shmem_base(rs);
	sbi_hart_map_saddr((unsigned long)shmem_base, rs->shmem.shmem_size);
	msgbuf = shmem_base;
	respbuf = resp_data_len ? shmem_base : NULL;

	ret = channel->send_message(channel, msg_id, msgbuf,
				    msg_data_len, respbuf, resp_data_len);
	sbi_hart_unmap_saddr();
	if (ret)
		return ret;
	...
	if (resp_data_len &&
	     ...
	     *resp_data_len > channel->attrs.msg_max_len))
