				  ...
				  void *rx, unsigned long *ack_len)
{
	...
	if (ack_len) {
		...
		else if (srv->max_rx_len < channel->attrs.msg_max_len)
			rx_len = srv->max_rx_len;
		else
			rx_len = channel->attrs.msg_max_len;
			  ...
			  const struct fdt_match *match)
{
	...
	/* FIXME: Get from message protocol implementation dynamically */
	rmb->channel.attrs.msg_max_len = RPMI_MSG_DATA_SIZE;
